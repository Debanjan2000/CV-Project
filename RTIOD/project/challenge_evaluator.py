...
for uid in targets.keys():
    #Convert Targets
    targets[uid]["boxes"] = torch.tensor(targets[uid]["boxes"], dtype=torch.float, device=device)
    targets[uid]["labels"] = torch.tensor(targets[uid]["labels"], dtype=torch.uint8, device=device)
    #Convert Submission
    submission[uid]["boxes"] = torch.tensor(submission[uid]["boxes"], dtype=torch.float, device=device)
    submission[uid]["labels"] = torch.tensor(submission[uid]["labels"], dtype=torch.uint8, device=device)
    if "scores" in submission[uid].keys():
        submission[uid]["scores"] = torch.tensor(submission[uid]["scores"], dtype=torch.float, device=device)
...
for entry in tqdm(targets.keys(), desc="Global"):
...
for key in metrics.keys():
    ...
    if key in ["map","map_50","map_75", "mar_1", "mar_10", "mar_100"]:
        scores[f'global_{key}'] = float(m_temp.cpu())
    if key in ["map_per_class", "mar_10_per_class"]:
        scores[f'global_{key}'] = list(m_temp.cpu().numpy().astype(float))
...
for name, id in intervals.items():
    print(f"## '{name}' ##")
    metric.reset()
    for entry in tqdm([x for x in targets.keys() if id in x[:8]], desc=f'Processing {name}'):
        metric.update([submission[entry]], [targets[entry]])
    ...
    for key in metrics.keys():
        m_temp = metrics[key]
        if key in ["map","map_50","map_75", "mar_1", "mar_10", "mar_100"]:
            scores[f'{name}_{key}'] = float(m_temp.cpu())
        if key in ["map_per_class", "mar_10_per_class"]:
            scores[f'{name}_{key}'] = list(m_temp.cpu().numpy().astype(float))
...
with open(os.path.join(args.output_dir, f'scores_{timestamp}.json'), 'w') as score_file:
